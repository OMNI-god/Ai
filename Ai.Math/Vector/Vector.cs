namespace Ai.Math.Vector
{
    public class Vector
    {
        ...
        {
            if (Magnitude() == 0 || other.Magnitude() == 0) throw new InvalidOperationException("Cannot normalize a zero vector.");
        ...
        {
            IsValid(this, other);
            double[] res = new double[other.Length];
            for (int i = 0; i < this.Length; i++)
            {
        ...
        public Matrix.Matrix Outer(Vector other)
        {
            double[,] res = new double[this.Length, other.Length];
            for (int row = 0; row < this.Length; row++)
            {
                for (int col = 0; col < other.Length; col++)
                {
                    res[row, col] = this[row] * other[col];
                }
            }
            return new Matrix.Matrix(res);
        }
    }
}
